...
export function createDeviceRoutes(controller: DeviceController): Router {
  ...
  router.get('/devices', [
    ...
    query('status').optional().isIn(['active', 'inactive', 'maintenance', 'error', 'online', 'offline']).withMessage('Invalid status'),
  ...
  router.put('/devices/:id', [
    ...
    body('status').optional().isIn(['active', 'inactive', 'maintenance', 'error', 'online', 'offline']).withMessage('Invalid status'),
    body('groupId').optional().isUUID().withMessage('Invalid groupId format'),
  ...
  router.post('/devices/:id/status', [
    param('id').isUUID().withMessage('Invalid device ID format'),
    body('status').isIn(['active', 'inactive', 'maintenance', 'error', 'online', 'offline']).withMessage('Invalid status'),
   ...
   */
  router.post('/devices/:id/test-connection', [
    param('id').isUUID().withMessage('Invalid device ID format')
