...
export class DeviceManagementService {
  ...
  async deleteDevice(id: string): Promise<void> {
    try {
      ...
      await this.cacheService.invalidateDeviceCache();
      // 이벤트 발행 (기존 패턴 사용)
      // await this.eventBusService.publishDeviceDeleted(id, existingDevice); // TODO: Fix event format
  ...
  /**
   * 장비 연결 테스트 - 실제 연결을 시도하여 접근성 확인
   */
  async testConnection(id: string): Promise<ConnectionTestResult> {
    try {
      this.logger.info('Testing device connection', { deviceId: id });

      // 1. Storage Service에서 장비 정보 조회
      const device = await this.storageClient.getDeviceById(id);
      if (!device) {
      ...
      if (!device.connectionInfo) {
        throw new Error(`Device '${device.name}' has no connection information configured`);
      }

      // 3. 암호화된 연결 정보 복호화
      const decryptedConnectionInfo = await this.storageClient.getDecryptedConnectionInfo(id);
      if (!decryptedConnectionInfo) {
        throw new Error('Failed to decrypt connection information');
      }

      // 4. 실제 연결 테스트 수행
      const result = await this.connectionTestService.testConnection(decryptedConnectionInfo);

      // 5. 테스트 결과를 이벤트로 발행
      await this.eventBusService.publishDeviceHealthCheck(id, {
      ...
      // 6. 연결 성공 시 장비 상태 업데이트
      if (result.success) {
        // Redis 캐시 업데이트
        await this.cacheService.setDeviceStatus(id, 'active');
        
        // DB에도 상태 업데이트 (Storage API 통해서)
        try {
          await this.storageClient.updateDevice(id, { status: 'active' });
          this.logger.info('Device status updated to active in DB', { deviceId: id });
        } catch (updateError) {
          this.logger.error('Failed to update device status in DB', { deviceId: id, error: updateError });
        }
      } else {
        // Redis 캐시 업데이트
        await this.cacheService.setDeviceStatus(id, 'error');
        
        // DB에도 상태 업데이트 (Storage API 통해서)
        try {
          await this.storageClient.updateDevice(id, { status: 'error' });
          this.logger.info('Device status updated to error in DB', { deviceId: id });
        } catch (updateError) {
          this.logger.error('Failed to update device status in DB', { deviceId: id, error: updateError });
        }
      }
      ...
      this.logger.error('Connection test failed', { deviceId: id, error });
